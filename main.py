def appendFile(file, text) -> None:
    "Adiciona informações ao arquivo saida.csv, é usado para a exportação"
    arq = open(file,"a")
    arq.write(text)
    arq.close()
...
class Program:
    ...
    def export(self) -> None:
        ...
        """
        clearFiles()
        appendFile("saida.csv","Token;Identificação;Tamanho;Posição\n")
        for i in self.tokens:
            appendFile("saida.csv",f"{i.token};{str(i.identification)};{i.size};{str(i.position)}\n")
